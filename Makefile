...
test: ## Run PHPUnit
	docker run --rm -v $$(pwd):/app php-dev composer dump-env test && ./vendor/bin/phpunit
...
test-simple: ## Run PHPUnit (simple)
	docker run --rm -v $$(pwd):/app php:8.3-cli composer dump-env test && ./vendor/bin/phpunit
